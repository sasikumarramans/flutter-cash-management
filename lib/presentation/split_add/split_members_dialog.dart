...
class SplitMembersDialog extends StatefulWidget {
  const SplitMembersDialog({super.key});

  @override
  State<SplitMembersDialog> createState() => _AddMemberDialogState();
}

class _AddMemberDialogState extends State<SplitMembersDialog> {
  ...
  void initState() {
    ...
    _searchController.text =
        GetIt.I.get<AddExpenseSplitBloc>().state.searchQuery ?? "";
  }
        ...
        return Container(
          ...
          child: Column(
            children: [
              ...
              _buildSearchBar(),
              if (state.participants.isNotEmpty) _buildSelectedUsers(state),
              SizedBox(
                height: state.participants.isEmpty ? 10 : 0,
              ),
  ...
  Widget _buildSelectedUsers(AddExpenseSplitState state) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: state.participants.map((user) {
              return Container(
                padding:
                ...
                child: Row(
                  ...
                  children: [
                    SizedBox(
                      ...
                      height: 30,
                      child: ProfileAvatar(
                        profilePicture: user.user.profileImageUrl,
                    ...
                    const SizedBox(width: 8),
                    GestureDetector(
                      onTap: () => _bloc.add(RemoveParticipant(user.user.id)),
                      child: const Icon(
                        Icons.close,
  ...
  Widget _buildSearchResults(AddExpenseSplitState state) {
    ...
    return Expanded(
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final isSelected =
              state.participants.any((u) => u.user.id == user.id);

          return InkWell(
            onTap: () {
              if (!isSelected) {
                _bloc.add(AddParticipant(user));
              }
            },
            child: Column(
              children: [
  ...
  Widget _buildSubmitButton(BuildContext context, AddExpenseSplitState state) {
    ...
    return Padding(
      ...
      child: AppButton(
        ...
        expandButton: true,
        buttonState: state.participants.isNotEmpty
            ? ButtonState.enabled
